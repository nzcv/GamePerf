...
impl AwesomeRpc {
    pub fn new(allowed_origins: Vec<&str>) -> Self {
        let port = 18554;//portpicker::pick_unused_port().expect("failed to get unused port for invoke");
        let allowed_origins =
            DomainsValidation::AllowOnly(allowed_origins.iter().map(|i| i.into()).collect());
        println!("port: {}", port);
        Self { port, allowed_origins }
    }
    ...
    pub fn start(&self) {
        ...
        tokio::spawn(async { server.wait().unwrap() });
        //tokio::async_runtime::spawn(async { server.wait().unwrap() });
    }
...
impl AwesomeEmit {
    ...
    pub fn send<P: Serialize>(&self, payload: P) {
        self.broadcaster.send(serde_json::to_string(&payload).unwrap()).unwrap();
    }
